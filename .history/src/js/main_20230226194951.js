...
// pjax? spa?
// ------------------------------
import { page1 } from "./views/about.js";
import { page2 } from "./views/skill.js";
import { page3 } from "./views/works.js";
import { page4 } from "./views/info.js";
...
const updateView = () => {
  ...
  const render = page || `<p class="textHidden">404 : Not Found<p>`;
...
if (
  ...
  stalker.classList.add("js-noneStalker");
}
...
// skill
// ------------------------------
// if ("/skill" == window.location.pathname) {
...
// works slider
// ------------------------------
// if ("/works" == window.location.pathname) {
...
const meteo = async () => {
  try {
    const res = await axios.get(
      "https://api.open-meteo.com/v1/forecast?latitude=43.07&longitude=141.35&daily=temperature_2m_max,temperature_2m_min&timezone=Asia%2FTokyo"
    );
    const w1 = res.data.daily.temperature_2m_max[0];
    const w2 = res.data.daily.temperature_2m_min[0];
    return console.log(w1 , w2);
  } catch (error) {
    return console.log(error);
  }
};
const append = async () => {
  const jokeText = await meteo();
  const weatherHigh = document.querySelector(".weatherBox .high");
  const weatherLow = document.querySelector(".weatherBox low");
  weatherHigh.append(w1);
  weatherLow.append(w2);
}


addEventListener('load',append);
